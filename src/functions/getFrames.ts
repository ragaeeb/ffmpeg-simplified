 ...
 */
export const getFrames = async (videoFile: string, options: GetFramesOptions, logger?: Logger): Promise<Frame[]> => {
    ...
    try {
        ...
        const framePaths = await collectFramePaths(options.outputFolder, filePrefix, fileExtension, options.frequency);
