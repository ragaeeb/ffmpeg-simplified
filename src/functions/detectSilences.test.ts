...
describe('detectSilences', () => {
    describe('mapOutputToSilenceResults', () => {
        ...
        it('handles floating-point values', () => {
            ...
            expect(mapOutputToSilenceResults(lines)).toEqual([{ end: 4.75, start: 3.5 }]);
        });

        it('handles integer values', () => {
            const lines = ['silence_start: 10', 'silence_end: 12'];
            expect(mapOutputToSilenceResults(lines)).toEqual([{ end: 12, start: 10 }]);
        });
        ...
        it('ignores intervals where end <= start', () => {
            expect(mapOutputToSilenceResults(['silence_start: 5', 'silence_end: 5'])).toEqual([]);
            expect(mapOutputToSilenceResults(['silence_start: 6', 'silence_end: 5'])).toEqual([]);
        });

        it('handles multiple silence intervals in sequence', () => {
            const lines = ['silence_start: 1', 'silence_end: 2', 'silence_start: 3.3', 'silence_end: 4.4'];
            expect(mapOutputToSilenceResults(lines)).toEqual([
                { end: 2, start: 1 },
                { end: 4.4, start: 3.3 },
            ]);
        ...
        it('overrides previous start if a new one appears before end', () => {
            ...
            expect(mapOutputToSilenceResults(lines)).toEqual([{ end: 3, start: 2 }]);
        });
    });
