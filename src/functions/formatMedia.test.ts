...
describe('formatMedia', () => {
	...
	it('should call ffmpeg with the correct arguments when noiseReduction is enabled with custom options', async () => {
		...
		expect(callbacks.onPreprocessingStarted).toHaveBeenCalledTimes(1);
		expect(callbacks.onPreprocessingStarted).toHaveBeenCalledWith(expect.any(String));
		expect(callbacks.onPreprocessingFinished).toHaveBeenCalledTimes(1);
	...
	it('should work with absolute paths', async () => {
		// Use os.tmpdir() to create absolute paths
		const tmpDir = os.tmpdir();
		const absoluteInputPath = path.join(tmpDir, `formatMedia-test-input-${Date.now()}.mp3`);
		const absoluteOutputPath = path.join(tmpDir, `formatMedia-test-output-${Date.now()}.mp3`);

		try {
			// Copy the sample file to the tmp directory
			const sampleFile = process.env.SAMPLE_MP3_FILE as string;
			await fs.promises.copyFile(sampleFile, absoluteInputPath);

			// Run formatMedia with absolute paths
			const outputPath = await formatMedia(absoluteInputPath, absoluteOutputPath, {
				noiseReduction: {
					afftdn_nf: -25,
					afftdnStart: 0.5,
					afftdnStop: 2,
			...
			try {
				await fs.promises.unlink(absoluteInputPath);
			} catch {
				// Ignore if file doesn't exist
			}
			try {
				await fs.promises.unlink(absoluteOutputPath);
			} catch {
				// Ignore if file doesn't exist
			}
		}
